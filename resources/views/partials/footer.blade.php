<div class="footer-grid">
  <ul class="footer-grid__item f_top f_row_ul">
    <li><a href="#" >Volver Arriba</a></li>
    <li><a href="https://www.facebook.com/amarilynmoda" class="fa fa-facebook"></a>
      <a href="https://www.twitter.com/amarilynmoda" class="fa fa-twitter"></a>
      <a href="https://www.instagram.com/amarilynmoda" class="fa fa-instagram"></a></li>
  </ul>

  <div class="footer-nav footer-grid__item f_left">
    <h6 class="footer-nav__title">Nosotros</h6>
    <ul class="footer-nav__submenu">
      <li><a class="footer-nav__link" href="#">Acerca de nosotros</a></li>
      <li><a class="footer-nav__link" href="#">Trabaja con nosotros</a></li>
      <li><a class="footer-nav__link" href="#">Contáctanos</a></li>
    </ul>
  </div>

  <div class="footer-nav footer-grid__item f_center">
    <h6 class="footer-nav__title">Información</h6>
    <ul class="footer-nav__submenu">
      <li><a class="footer-nav__link" href="#">Preguntas frecuentes</a></li>
      <li><a class="footer-nav__link" href="#">Políticas de alquiles y venta</a></li>
      <li><a class="footer-nav__link" href="#">Términos y condiciones</a></li>
    </ul>
  </div>

  <div class="footer-nav footer-grid__item f_right">
    <h6 class="footer-nav__title">Encuéntranos</h6>
    <p>Cra 17 No. 36-41, CC Omnicentro, Piso 2, Local 1B-14, B/manga</p>
    <a href="#" class="footer-misc__tiendas">
      <i class="fas fa-map-marker-alt"></i>
      <span> Localiza nuestra tienda</span>
    </a>
  </div>

  <div class="footer-grid__item f_bottom">© <span class="footer-creditos__year">2019 &nbsp</span>  Marilyn Moda ltda.</div>

</div> <!--END FOOTER-GRID-->
